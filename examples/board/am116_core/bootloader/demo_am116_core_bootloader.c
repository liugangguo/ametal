...
static am_softimer_t startup_wait_timer;
...
static void __startup_wait_timer_handle(void *p_arg)
{
    if(timer != 0){
        timer--;
        if(timer == 0) {
            am_softimer_stop(&startup_wait_timer);
            timer = BOOT_WAITE_TIME;
...
static int __read_data(uint8_t *p_buffer, uint32_t byte_count)
{
    ...
    am_softimer_start(&receive_callback_timer, 15000);
...
void demo_am116_core_bootloader_entry (void)
{
    ...
    /* 启动延时回调定时器初始化  */
    am_softimer_init(&startup_wait_timer, __startup_wait_timer_handle, NULL);
    ...
    while(1) {
        ...
        /* 开始固件存放  */
        am_boot_firmware_store_start(firmware_handle, 22 * 1024);
        /* 切换为固件接收状态  */
