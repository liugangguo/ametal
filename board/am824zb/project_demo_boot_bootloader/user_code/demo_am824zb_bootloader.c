...
void demo_am824zb_bootloader_entry (void)
{
    ...
    am_zm516x_cfg_info_set(zm516x_handle, &zm516x_cfg_info);
    ...
    while(1) {
	    ...
	    timer = 0;
		 ...
		 if (boot_state == BOOT_STARTUP_STATE) {
			 	 ...
			 	 /* 准备固件存放 */
         am_boot_firmware_store_start(firmware_handle, 17 * 1024);
        ...
        am_kprintf("bootloader : firmware receive successful %d\r\n", count + 8 - 2);
